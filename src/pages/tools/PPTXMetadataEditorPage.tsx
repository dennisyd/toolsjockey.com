...
const PPTXMetadataEditorPage: React.FC = () => {
  ...
  const handleFile = (f: File) => {
    ...
    setError(null);
    setProgress(0);
    // TODO: Implement PPTX metadata editing logic
    // Simulate progress
    let prog = 0;
  ...
  return (
    <div className="container-app mx-auto px-4 py-8 max-w-2xl">
      <h1 className="text-2xl font-bold mb-4">PPTX Metadata Editor</h1>
      <div
      ...
      {progress > 0 && progress < 100 && (
        <div className="mb-4"><div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-accent h-2 rounded-full" style={{ width: `${progress}%` }}></div></div><div className="text-xs mt-1">Processing... {progress}%</div></div>
      )}
      {error && <div className="text-red-600 mb-4">{error}</div>}
      {/* TODO: Add metadata editing UI */}
    </div>
  );
};

export default PPTXMetadataEditorPage; 