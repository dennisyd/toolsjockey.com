...
const VolumeNormalizerPage: React.FC = () => {
  ...
  const [targetDb, setTargetDb] = useState<number>(-1);
  ...
  const handleFile = (f: File) => {
    ...
    setProgress(0);
    setOutputUrl(null);
    // TODO: Implement volume normalization using Web Audio API/Tone.js
    // Simulate progress
    let prog = 0;
  ...
  return (
    <div className="container-app mx-auto px-4 py-8 max-w-2xl">
      <h1 className="text-2xl font-bold mb-4">Volume Normalizer</h1>
      <div
      ...
      <div className="mb-4 flex gap-4 items-center">
        <label className="block text-sm font-medium">Target dB:
          <input type="number" value={targetDb} onChange={e => setTargetDb(Number(e.target.value))} className="ml-2 p-1 border rounded w-20" />
        </label>
      </div>
      ...
      {progress > 0 && progress < 100 && (
        <div className="mb-4"><div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-accent h-2 rounded-full" style={{ width: `${progress}%` }}></div></div><div className="text-xs mt-1">Normalizing... {progress}%</div></div>
      )}
      {error && <div className="text-red-600 mb-4">{error}</div>}
      {outputUrl && <a href={outputUrl} download="normalized.mp3" className="btn btn-accent">Download Normalized Audio</a>}
    </div>
  );
};
