...
const PPTXExtractTextPage: React.FC = () => {
  ...
  const handleFile = (f: File) => {
    ...
    setError(null);
    setProgress(0);
    // TODO: Implement PPTX text extraction using pptxgenjs or similar
    // Simulate progress
    let prog = 0;
    const interval = setInterval(() => {
      ...
      if (prog >= 100) {
        ...
        setText('Sample extracted text from slides...'); // Placeholder
      }
    }, 200);
  };
  ...
  return (
    <div className="container-app mx-auto px-4 py-8 max-w-2xl">
      <h1 className="text-2xl font-bold mb-4">Extract Text from PPTX</h1>
      <div
      ...
      {progress > 0 && progress < 100 && (
        <div className="mb-4"><div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-accent h-2 rounded-full" style={{ width: `${progress}%` }}></div></div><div className="text-xs mt-1">Extracting... {progress}%</div></div>
      )}
      {error && <div className="text-red-600 mb-4">{error}</div>}
      {text && (
        <div className="mt-4 bg-gray-100 dark:bg-gray-800 rounded p-4 whitespace-pre-wrap text-sm">
          {text}
        </div>
      )}
    </div>
...
export default PPTXExtractTextPage; 