...
import { Play, Download, Upload, Settings, RotateCcw } from 'lucide-react';
...
const AudioSpeedChangerPage: React.FC = () => {
  ...
  useEffect(() => {
    document.title = 'Audio Speed Changer – ToolsJockey';
    const meta = document.querySelector('meta[name="description"]');
    if (meta) meta.setAttribute('content', 'Change the playback speed of audio files from 0.25x to 3x with real-time preview.');
  }, []);
  ...
  const handleFile = async (f: File) => {
    ...
    try {
      ...
      setProgress(100);
      trackButtonClick('audio_file_loaded', 'AudioSpeedChanger');
    } catch (e: any) {
  ...
  const changeSpeed = async () => {
    ...
    try {
      ...
      trackButtonClick('audio_speed_changed', 'AudioSpeedChanger');
    } catch (e: any) {
      setError('Failed to change audio speed: ' + e.message);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const downloadAudio = () => {
    ...
    link.href = outputUrl;
    const baseName = file.name.split('.')[0] || 'speed-changed';
    link.download = `${baseName}_${speed}x.wav`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    trackButtonClick('audio_downloaded', 'AudioSpeedChanger');
  };
  ...
  const handleAudioTimeUpdate = () => {
    if (audioRef.current) {
      // setCurrentTime(audioRef.current.currentTime); // This line was removed
    }
  };
  ...
  return (
    <div className="container-app mx-auto px-4 py-8 max-w-4xl">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold mb-2">Audio Speed Changer</h1>
        <p className="text-gray-600 dark:text-gray-300">
          Change the playback speed of audio files from 0.25x to 3x
        </p>
      </div>
          ...
          <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <audio
                ...
                // onPlay={() => setIsPlaying(true)} // This line was removed
                // onPause={() => setIsPlaying(false)} // This line was removed
              />
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                New duration: {formatTime(duration / speed)}
              </p>
            </div>
            <div>
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <p className="text-sm">
                  <strong>Original:</strong> {formatTime(duration)}
                </p>
                <p className="text-sm">
                  <strong>Speed {speed}x:</strong> {formatTime(duration / speed)}
                </p>
                <p className="text-sm">
                  <strong>Time saved:</strong> {formatTime(duration - (duration / speed))}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-primary-light rounded-lg shadow-lg p-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Speed: {speed}x • Duration: {formatTime(duration / speed)}
              </p>
            </div>
            <button
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-primary-light rounded-lg p-6 text-center">
          ...
          <h3 className="font-semibold mb-2">Speed Control</h3>
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Change speed from 0.25x to 3x with precise control
          </p>
        </div>
        <div className="bg-white dark:bg-primary-light rounded-lg p-6 text-center">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            See duration changes and time savings instantly
          </p>
        </div>
        <div className="bg-white dark:bg-primary-light rounded-lg p-6 text-center">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Maintain audio quality while changing speed
          </p>
        </div>
      </div>
    </div>
