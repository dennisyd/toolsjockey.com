...
import { Download, Upload, Settings, Scissors } from 'lucide-react';
...
const AudioClipperPage: React.FC = () => {
  ...
  useEffect(() => {
    document.title = 'Audio Clipper – ToolsJockey';
    const meta = document.querySelector('meta[name="description"]');
    if (meta) meta.setAttribute('content', 'Extract a segment from an audio file.');
  }, []);
  ...
  const handleFile = async (f: File) => {
    ...
    try {
      ...
      setStart(0);
      setEnd(Math.min(10, buffer.duration));
      setProgress(100);
      trackButtonClick('audio_file_loaded', 'AudioClipper');
    } catch (e: any) {
  ...
  const clipAudio = async () => {
    ...
    try {
      ...
      const clipStart = Math.max(0, Math.min(start, audioBuffer.duration - 0.01));
      const clipEnd = Math.max(clipStart, Math.min(end, audioBuffer.duration));
      const clipLength = Math.floor((clipEnd - clipStart) * sampleRate);
      const clippedBuffer = audioContext.createBuffer(numberOfChannels, clipLength, sampleRate);
      for (let channel = 0; channel < numberOfChannels; channel++) {
        const channelData = audioBuffer.getChannelData(channel).slice(
          Math.floor(clipStart * sampleRate),
          Math.floor(clipEnd * sampleRate)
        );
        clippedBuffer.getChannelData(channel).set(channelData);
      }
      setProgress(70);
      const audioData = await convertToWav(clippedBuffer);
      setProgress(90);
      const blob = new Blob([audioData], { type: 'audio/wav' });
      const url = URL.createObjectURL(blob);
      setOutputUrl(url);
      setProgress(100);
      trackButtonClick('audio_clipped', 'AudioClipper');
    } catch (e: any) {
      setError('Failed to clip audio: ' + e.message);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const downloadAudio = () => {
    ...
    link.href = outputUrl;
    const baseName = file.name.split('.')[0] || 'clipped';
    link.download = `${baseName}_clip.wav`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    trackButtonClick('audio_downloaded', 'AudioClipper');
  };
  ...
  return (
    <div className="container-app mx-auto px-4 py-8 max-w-4xl">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold mb-2">Audio Clipper</h1>
      ...
      <div className="bg-white dark:bg-primary-light rounded-lg shadow-lg p-6 mb-6">
        ...
        {file && <div className="text-sm text-gray-500 mt-2">{file.name}</div>}
      </div>
      {file && (
        <div className="bg-white dark:bg-primary-light rounded-lg shadow-lg p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
          ...
          <div className="mt-2 text-xs text-gray-500">
            Duration: {formatTime(end - start)} (of {formatTime(duration)})
          </div>
        </div>
      )}
      {file && (
      ...
      {outputUrl && (
        <div className="bg-white dark:bg-primary-light rounded-lg shadow-lg p-6">
          <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
            ...
            Download Clipped Audio
          </h2>
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium">Clip Complete!</p>
              <p className="text-sm text-gray-500">
                Start: {formatTime(start)} • End: {formatTime(end)} • Duration: {formatTime(end - start)}
              </p>
            </div>
            <button
